package finter.gui;

public class Textos {

	public static final String AGREGAR_PUNTO = "Agregar punto";
	public static final String OK = "OK";
	public static final String CANCELAR = "Cancelar";
	public static final String X = "X: ";
	public static final String Y = "Y: ";
	public static final String ERROR_X = "Error en X";
	public static final String ERROR_Y = "Error en Y";
	public static final String CALCULAR_POLINOMIO = "Calcular polinomio";
	public static final String LAGRANGE = "Lagrange";
	public static final String NG_P = "Newton-Gregory (Progresivo)";
	public static final String NG_R = "Newton-Gregory (Regresivo)";
	public static final String CALCULAR = "Calcular";
	public static final String ESPECIALIZAR_POLINOMIO = "Especializar polinomio";
	public static final String ESPECIALIZAR = "Especializar";
	public static final String VOLVER = "Volver";
	public static final String Q_RESTAURAR_PUNTOS = "\u00BFRestaurar puntos del último polinomio calculado?";
	public static final String SI = "SI";
	public static final String NO = "NO";
	public static final String Q_SALIR = "\u00BFSalir de FINTER?";
	public static final String VER_PASOS = "Ver pasos de c\u00E1lculo";
	public static final String PASOS = "Pasos de c\u00E1lculo:";
	public static final String FINTER = "FINTER";
	public static final String POLINOMIO_ENCONTRADO = "Polinomio encontrado:";
	public static final String PUNTOS_A_TOMAR = "Puntos a tomar:";
	public static final String PLUS_SIGN = "+";
	public static final String MINUS_SIGN = "-";
	public static final String I_LABEL = "i";
	public static final String X_LABEL = "X";
	public static final String Y_LABEL = "Y";
	public static final String LESS_THAN_SIGN = "<";
	public static final String SALIR = "Salir";
	public static final String EXISTE_X = "Ya existe esa X";
	public static final String METODO = "M\u00E9todo:";
	public static final String GRADO = "Grado:";
	public static final String PUNTOS_EQUIESPACIADOS = "Puntos equiespaciados: ";
	public static final String FUERA_DE_RANGO = "X debe estar entre puntos definidos";
	public static final String PASO = "Paso";
	
	
}
